// Extracted message NpcTalkReq
// Tool: OverfieldTool

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        v6.31.1
// source: NpcTalkReq.proto

package proto

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type NpcTalkReq struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            uint32                 `protobuf:"varint,1,opt,name=id,proto3" json:"id,omitempty"`
	TalkType      NpcTalkType            `protobuf:"varint,2,opt,name=talk_type,json=talkType,proto3,enum=proto.NpcTalkType" json:"talk_type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *NpcTalkReq) Reset() {
	*x = NpcTalkReq{}
	mi := &file_NpcTalkReq_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *NpcTalkReq) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NpcTalkReq) ProtoMessage() {}

func (x *NpcTalkReq) ProtoReflect() protoreflect.Message {
	mi := &file_NpcTalkReq_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NpcTalkReq.ProtoReflect.Descriptor instead.
func (*NpcTalkReq) Descriptor() ([]byte, []int) {
	return file_NpcTalkReq_proto_rawDescGZIP(), []int{0}
}

func (x *NpcTalkReq) GetId() uint32 {
	if x != nil {
		return x.Id
	}
	return 0
}

func (x *NpcTalkReq) GetTalkType() NpcTalkType {
	if x != nil {
		return x.TalkType
	}
	return NpcTalkType_NpcTalkType_Npc
}

var File_NpcTalkReq_proto protoreflect.FileDescriptor

const file_NpcTalkReq_proto_rawDesc = "" +
	"\n" +
	"\x10NpcTalkReq.proto\x12\x05proto\x1a\x11NpcTalkType.proto\"M\n" +
	"\n" +
	"NpcTalkReq\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\rR\x02id\x12/\n" +
	"\ttalk_type\x18\x02 \x01(\x0e2\x12.proto.NpcTalkTypeR\btalkTypeB\n" +
	"Z\b./;protob\x06proto3"

var (
	file_NpcTalkReq_proto_rawDescOnce sync.Once
	file_NpcTalkReq_proto_rawDescData []byte
)

func file_NpcTalkReq_proto_rawDescGZIP() []byte {
	file_NpcTalkReq_proto_rawDescOnce.Do(func() {
		file_NpcTalkReq_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_NpcTalkReq_proto_rawDesc), len(file_NpcTalkReq_proto_rawDesc)))
	})
	return file_NpcTalkReq_proto_rawDescData
}

var file_NpcTalkReq_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_NpcTalkReq_proto_goTypes = []any{
	(*NpcTalkReq)(nil), // 0: proto.NpcTalkReq
	(NpcTalkType)(0),   // 1: proto.NpcTalkType
}
var file_NpcTalkReq_proto_depIdxs = []int32{
	1, // 0: proto.NpcTalkReq.talk_type:type_name -> proto.NpcTalkType
	1, // [1:1] is the sub-list for method output_type
	1, // [1:1] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_NpcTalkReq_proto_init() }
func file_NpcTalkReq_proto_init() {
	if File_NpcTalkReq_proto != nil {
		return
	}
	file_NpcTalkType_proto_init()
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_NpcTalkReq_proto_rawDesc), len(file_NpcTalkReq_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_NpcTalkReq_proto_goTypes,
		DependencyIndexes: file_NpcTalkReq_proto_depIdxs,
		MessageInfos:      file_NpcTalkReq_proto_msgTypes,
	}.Build()
	File_NpcTalkReq_proto = out.File
	file_NpcTalkReq_proto_goTypes = nil
	file_NpcTalkReq_proto_depIdxs = nil
}
